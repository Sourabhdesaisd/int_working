...
#
#* Last Modified : Sun 14 Jun 2026 11:11:03 AM IST
#
#* Created By :  
#
#///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#    cmd = ["xrun", "-access", "+rwc", "-f", "./compile.f", "-svseed", str(seed),
...
#          "-uvmhome", "CDNS-1.1d"]
    cmd = ["xrun", "-sv", "-access", "+rwc", "-f", "./compile.f", "-svseed", str(seed),
	"+UVM_TESTNAME={}".format(test), "+define+UVM_REPORT_DISABLE_FILE_LINE",
          "-uvmhome", "CDNS-1.1d"]

    if cov_en:
        cmd.extend(["-coverage", "all", "-covdut", "tb_top",
                    "-covworkdir", "/cov_work", "-covoverwrite",
                    "-covfile", "./cov_files/cov_cmd.cf"])

    subprocess.call(cmd)
    shutil.move("xrun.log", os.path.join(simdir, "xrun.log"))
...
def report_test(tests, regdir, date, cov_en):
    ...
    for test in tests:
        ...
        for seed in range(0, 100000):
            simdir = "{}/{}_{}".format(regdir, test, seed)
            log_file_path = os.path.join(simdir, "xrun.log")
