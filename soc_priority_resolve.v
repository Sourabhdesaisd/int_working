`timescale 1ns / 1ps
module soc_priority_resolve
(
input               soc_clk                     ,
input               soc_rst                     ,
input               wdt_reset_i                 ,
input       [15:0]  int_pending_i               ,
...
begin
	...
	begin
    highest_pending_lvl_pr_o     <= stage3_0    ;
    interrupt_id_o               <= stage_id3_0 ;
...
always @(*) begin
    $display("[%0t] pending=%h highest_lvl=%h highest_id=%0d",
             $time,
             int_pending_i,
...
always @(*) begin
  ...
  // Compare level bits first
  if (val_in_1[7:5] > val_in_2[7:5]) begin
    val_out = val_in_1;
    out_id  = val1_id;
  end
  else if (val_in_2[7:5] > val_in_1[7:5]) begin
    val_out = val_in_2;
    out_id  = val2_id;
  end

  // If level is same, compare priority bits
  else begin
    if (val_in_1[4:2] > val_in_2[4:2]) begin
      val_out = val_in_1;
      out_id  = val1_id;
    end
    else if (val_in_2[4:2] > val_in_1[4:2]) begin
      val_out = val_in_2;
      out_id  = val2_id;
    end

    // If level and priority are same, highest interrupt ID wins
    else begin
      if (val1_id > val2_id) begin
        val_out = val_in_1;
        out_id  = val1_id;
      end
      else begin
