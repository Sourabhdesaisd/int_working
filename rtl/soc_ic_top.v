...
module soc_ic_top
(
input 	       		soc_clk		        	,//global clock
input 	       		soc_rst		        	,//global reset
input               wdt_reset_i             ,
input 		[15:0]  interrupt_enable_i  		,//from memory mapped register
...
//input 	       		ext_int46_i	        	,
//input 	       		ext_int47_i	        	,
input 		[7:0] 	int0_lp_i	        	,//interrupt level priority value from soc mmr 
...
input 			soc_eoi_valid_i			,//end of interrupt valid signal from core/soc_mmr
input               	soc_ack_in          		,//interrupt request acknowledge signal from core/soc_mmr
input       	[7:0]  	soc_ack_id_in       		,//interrupt id of acknowledged interrupt
...
output  	[15:0]  interrupt_pending_o		,//interrupt pending signal fr each interrupt line to soc_mmr
output 			interrupt_pending_valid_o	,
output 			soc_nxtp_valid_o	 	,//from soc to reg file
output 		[7:0] 	highest_pending_lvl_pr_o 	,
output  	[7:0] 	soc_nxtp_id_o			, //from soc to reg file
...
assign soc_nxtp_valid_o = highest_pending_lvl_pr_valid_w;
assign soc_nxtp_id_o    = interrupt_id_w		;

soc_int_rec interrupt_recieve_inst
(
.soc_clk		   	            (soc_clk	    		            ),
.soc_rst		   	            (soc_rst	    		            ),
.wdt_reset_i                    (wdt_reset_i                        ),
.ack_in                         (soc_ack_in                         ),
.ack_id                         (soc_ack_id_in                      ),
...
soc_priority_resolve priority_resolve_inst
(
.soc_clk			            (soc_clk			                ),
.soc_rst			            (soc_rst			                ),
.wdt_reset_i                    (wdt_reset_i                        ),
...
soc_irg soc_irg_inst
(
//.soc_clk			            (soc_clk			                ),
//.soc_rst			            (soc_rst			                ),
//.wdt_reset_i                    (wdt_reset_i                        ),
.highest_pending_lvl_pr_i	    (highest_pending_lvl_pr_w	        ),//from soc_pr block
//.highest_pending_lvl_pr_valid	(highest_pending_lvl_pr_valid_w	    ),
.active_lvl_pr_i 	    	    (active_lvl_pr_i		            ),//from CSR
.highest_pending_int_id_i		(interrupt_id_w			            ),//from soc_pr
.interrupt_request_o	    	(interrupt_request_o		        ),//to core
.interrupt_id_o		    	    (interrupt_id_o			            ),//to soc_ack_mmr
.interrupt_id_valid_o	    	(interrupt_id_valid_o		        ),//to soc_ack_mmr
//.soc_eoi_valid			        (soc_eoi_valid_i		            ), //end of interrupt signal from core
